...
import { BanknotesIcon, TicketIcon, GlobeAltIcon, PhotoIcon, BriefcaseIcon } from '@heroicons/react/24/outline';
...
const MovieDetailsScreen = () => {
  ...
  const bgImage = useMemo(() => TmdbService.getImageFullPath(movie.backdrop_path), [movie.id, i18n.language]);
  const posterImage = useMemo(() => TmdbService.getImageFullPath(movie.poster_path), [movie.id, i18n.language]);
  const releaseDate = useMemo(() => CustomizeService.printDate(movie.release_date), [movie.id, i18n.language]);
  ...
  return (
    ...
    <div className='transition-all ease-in-out max-h-36 group hover:max-h-96 overflow-hidden flex justify-center items-center'>
      {
        (movie.backdrop_path && <img src={bgImage} className='transition-all object-cover' />) ||
    ...
    <div className='container px-2 sm:px-auto mx-auto'>
      <div className='flex flex-wrap justify-center my-6'>
        {
          (movie.poster_path && 
          <motion.img src={posterImage} className='h-full w-32 sm:w-64 mx-4 rounded-md shadow-md'
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
        ...
        <div className='flex-1 flex flex-wrap flex-col mx-2'>
          ...
          <p className='flex flex-wrap align-baseline text-sm text-neutral-700 my-2'>
            <div className='pr-2 py-1'>
              {
                (movie.runtime && CustomizeService.formatTime(movie.runtime)) ||
                <div className='h-5 w-16 bg-gray-200 rounded-md'></div>
              }
            </div>
            <span className={`pr-2 py-1 ${CustomizeService.getClassScore(movie.vote_average)}`}>
              {
      ...
      <div className='flex flex-wrap justify-center align-baseline border-t-slate-800 border-t-2 pt-6'>
        {
          movie.production_companies && movie.production_companies.sort((a, b) => a.name.localeCompare(b.name)).map((company) => 
